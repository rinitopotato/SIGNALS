...
export const METRIC_INFO = {
  ...
  IC:  { formula: 'IC = Pearson(signal_t, ΔP_{t+k*})', description: 'Information Coefficient is the Pearson correlation between a trader\'s signal (or position) and the subsequent price change at the optimal lag k*. IC > 0.05 is considered economically significant.', source: 'Cowgill & Zitzewitz (2015); NCM (2009)' },
  ...
  BS:  { formula: 'BS = (1/N) Σ(fᵢ − oᵢ)²', description: 'Brier Score measures probabilistic forecast accuracy. Lower is better. BS < 0.20 = skilled, 0.20–0.35 = moderate, > 0.35 = poor. Penalizes overconfident wrong forecasts heavily.', source: 'Brier (1950)' },
  ...
  // Lens 2 — Social Capital
  J:   { formula: 'J(i,j) = |M_i ∩ M_j| / |M_i ∪ M_j|', description: 'Jaccard Similarity measures co-participation overlap between two traders across all markets. J = 1 means they trade in exactly the same markets; J = 0 means no overlap.', source: 'Jaccard (1901)' },
  ...
  FM:  { formula: 'FM_i = ΔParticipation after i\'s trade / baseline', description: 'Follower Multiplier measures how many additional trades from other wallets follow within 30 minutes of trader i\'s trade, indicating influence or information leadership.', source: 'Golub & Jackson (2010)' },
  ...
  // Lens 3 — Signal Engineering
  IT:  { formula: 'I_t = w₁·Z(P_{BOJ}) + w₂·Z(A_t)', description: 'Composite Signal Index combines the z-scored BOJ prediction market price and attention proxy with weights optimized by IC. I_t > 0 signals HIKE regime.', source: 'NCM (2009)' },
  ...
  WFA: { formula: 'WFA = mean(accuracy on out-of-sample folds)', description: 'Walk-Forward Accuracy averages the directional accuracy of the signal on 5 rolling out-of-sample test windows. WFA > 0.55 suggests genuine predictive power.', source: 'Lopez de Prado (2018)' },
  ...
  MCP: { formula: 'MCP = P(IC > 0 | Monte Carlo resample)', description: 'Monte Carlo Signal Pass is the fraction of 500 bootstrap resamples in which the ensemble signal achieves positive IC. MCP > 0.90 means the signal is statistically robust.', source: 'Lopez de Prado (2018)' },
